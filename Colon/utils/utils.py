...
# added seed handling and MCAR

_rng_instance = None
def set_column_seed(column_seed):
    global _rng_instance
    _rng_instance = np.random.default_rng(column_seed)

def set_sample_seed(sample_seed):
    torch.manual_seed(sample_seed)
    random.seed(sample_seed)

def MCAR_mask(X, p_miss):
    return (torch.rand(X.shape) < p_miss)
...
def pick_epsilon(X, quant=0.5, mult=0.05, max_points=2000):
    ...
    idx = _rng_instance.choice(len(X_), min(max_points, len(X_)), replace=False)
    X = X_[idx]
...
def MAR_mask(X, p, p_obs):
    ...
    idxs_obs = _rng_instance.choice(d, d_obs, replace=False)
...
def MNAR_mask_logistic(X, p, p_params =.3, exclude_inputs=True):
    ...
    idxs_params = _rng_instance.choice(d, d_params, replace=False) if exclude_inputs else np.arange(d)
...
def MNAR_self_mask_logistic(X, p):
    ...
    ber = torch.rand(n, d) if to_torch else _rng_instance.rand(n, d)
...
def MNAR_mask_quantiles(X, p, q, p_params, cut='both', MCAR=False):
    ...
    idxs_na = _rng_instance.choice(d, d_na, replace=False) ### select at least one variable with missing values
