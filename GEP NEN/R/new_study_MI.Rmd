...
### Setting Not done and unknown to NA

```{r}
#df[df["Date of PT Resection"]   == "<NA>"] = NA
#df.raw[df.raw == "Not Done"] = NA
#df.raw[df.raw == "Unknown"] = NA
```

### Check constant columns

```{r}
cols_with_one_unique_value <- function(df) {
  sapply(df, function(x) length(unique(na.omit(x)))) == 1
}

cols_to_print <- cols_with_one_unique_value(df.raw)

# Print columns with only one unique value
print(df.raw[, cols_to_print])
```
...
imp = mice(df.mice, m = M, pred = predictor_matrix, me = me, seed = 124124)
```

400 logged events.. not good

```{r}
imp$loggedEvents
```

### Attempt to reduce columns by investigating the influx and outflux

```{r}
fluxplot(df.mice, eqscplot = FALSE)
```

A bit cluttered. Let's try to manually plot

```{r}
df.flux = flux(df.mice)
...
Try removing Differentiation,, Chromogranin_Atwo and CD_fiftysix
...
#low.flux.cols = c("Differentiation", "Chromogranin_Atwo")
#df.reduced <- subset(df.mice, select = !(names(df.mice) %in% low.flux.cols))

#keepNames <- setdiff(names(me), low.flux.cols)
#me.reduced <- me[keepNames]
```

```{r}
#predictor_matrix = quickpred(df.reduced, mincor = 0.3)
```
...
```{r}
imp = mice(df.mice, m = M, pred = predictor_matrix, me = "pmm", seed = 124124)
```
...
"On convergence, the streams should intermingle and be free of any trend"
